...
class OTAUpdater:
    ...
    def __init__(self, github_repo, module='', main_dir='main'):
      ...
      try:
        with open('config.json', 'r') as configfile:
          c = ujson.load(configfile)
          self.auto_update = c['AUTO_UPGRADE']
          self.only_releases = c['ONLY_RELEASES']
          self.maximum_version = c['MAXIMUM_VERSION']
          self.last_commit = c['last_commit']
          self.branch_commit = c['COMMIT_BRANCH']
          configfile.close()
        self.test()
    ...
    def test(self):
      ...
      print(self.only_releases)
      print(self.maximum_version)
    ...
    def autoUpdate(self, ssid, password):
      ...
      if self.auto_update:
        if self.only_releases:
          ...
          date = self.get_latest_commit()
          if(date == self.last_commit):
            print('Working with last commit!')
          else:
            if self.downloadInstall('Null'):
              with open('config.json', 'r') as configfile:
                c = ujson.load(configfile)
                configfile.close()
              ...
              with open('config.json', 'w') as configfile:
                configfile.write(ujson.dumps(c))
                configfile.close()
              
              print('Installed the commit with date:' + date)
              machine.reset()
            else:
              print('Troubles when installing, rebooting...')
              machine.reset()
      else:
        print("Auto update function isdiabled, Starting the memory program...")
        
    def downloadInstall(self, latest_version):
      self.rmtree(self.modulepath(self.main_dir))
      os.mkdir(self.modulepath(self.main_dir))
      self.download_all_files(self.github_repo + '/contents/' + self.main_dir, latest_version)

    def check_for_update_to_install_during_next_reboot(self):
      current_version = self.get_version(self.modulepath(self.main_dir))
      latest_version = self.get_latest_version()
      print('Checking version...\tCurrent version:', current_version, '\tLatest version: ', latest_version)
      if latest_version > current_version:
        print('New version available, willdownloadand install on next reboot')
        os.mkdir(self.modulepath('next'))
        with open(self.modulepath('next/.version on reboot'), 'w') as versionfile:
          versionfile.write(latest_version)
          versionfile.close()

    def download_and_install_update_if_available(self, ssid, password):
      if 'next' in os.litdir(self.module):
    ...
    def _download_and_install_update(self, latest_version, ssid, password):
      ...
      self.download_all_files(self.github_repo + '/contents/' + self.main_dir, latest_version)
      self.rmtree(self.moduepath(self.main_dir))
    ...
    def get_latest_commit(self):
      latest_commit = self.http_client.get(self.github_repo + '/commits/' + self.branch_commit)
      print(self.github_repo + '/commits/' + self.branch_commit)
      date = latest_commit.json()['commit']['author']['date']
      latest_commit.close()
      return date
   ...
   def request(self, method, url, data=None, json=None, headers={}, stream=None):
     ...
     try:
       ...
       if json is not None:
         ...
         s.write(b'Connect-Type: application/json\r\n')
       if data:
         s.write(b'Connect-Length: %d\r\n' % len(data))
       s.write(b'\r\n')
       if data:
         s.write(data)
